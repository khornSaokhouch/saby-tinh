      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1 text-left">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Dimension Management</span>
          </div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight leading-none">
            Product <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-blue-400">Sizes</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Plus size={16} strokeWidth={3} /> Add New Size
          </button>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <MetricCard label="Registered Sizes" value={sizes.length} icon={Ruler} color="indigo" />
        <MetricCard label="Active Units" value={sizes.filter(s => s.status === 'active').length} icon={CheckCircle2} color="emerald" />
        <MetricCard label="System Sync" value="Verified" icon={Clock} color="purple" />
      </div>
      ...
      <div className="bg-white rounded-[20px] border border-slate-100 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
        <div className="p-4 border-b border-slate-50 bg-slate-50/20 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative w-full sm:w-80 group">
            ...
            <input 
              type="text" 
              placeholder="Search by size label..." 
          ...
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
            {filteredSizes.length} Results Found
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                ...
                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Live Status</th>
              ...
              {loading && sizes.length === 0 ? (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center border border-slate-100 shadow-sm transition-all group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-600">
                        <Maximize size={14} />
                      </div>
