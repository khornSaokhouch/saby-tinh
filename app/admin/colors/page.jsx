...
export default function ColorsPage() {
  ...
  const handleBatchDelete = async () => {
    if (window.confirm(t(`Are you sure you want to delete ${selectedIds.length} colors?`, language))) {
      setIsActionLoading(true);
      try {
        await deleteMultipleColors(selectedIds);
        setSelectedIds([]);
        toast.success(t(`Removed ${selectedIds.length} chromatic units`, language));
      } catch (err) {
  ...
  const handleSave = async (data) => {
    ...
    try {
      ...
      setIsFormOpen(false);
      toast.success(t('Color palette synchronized', language));
    } catch (err) {
      console.error(err);
      toast.error(t('Failed to update palette', language));
    } finally {
      setIsActionLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.success(t('Chromatic unit removed', language));
    } catch (err) {
      console.error(err);
      toast.error(t('Failed to delete color', language));
    } finally {
      setIsActionLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-5 pb-8 font-sans max-w-[1400px] mx-auto animate-in fade-in duration-500 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="text-left">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">{t('Chromatic Registry', language)}</span>
          </div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tighter leading-none">
            {t('Product', language)} <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-emerald-400">{t('Colors', language)}</span>
          </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Plus size={14} strokeWidth={3} /> {t('Create Color', language)}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <MetricCard label={t('Total Colors', language)} value={colors.length} icon={Palette} color="indigo" />
        <MetricCard label={t('Visual Balance', language)} value={colors.filter(c => c.status === 'active').length} icon={CheckCircle2} color="emerald" subText={t('Active', language)} />
        <MetricCard label={t('Registry Status', language)} value={t('Synced', language)} icon={Clock} color="purple" />
      </div>
        ...
        <div className="p-4 border-b border-slate-50 bg-slate-50/20 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
            <div className="relative w-full sm:w-64 group text-left">
              ...
              <input
                type="text"
                placeholder={t('Search colors...', language)}
          ...
          <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">
            {filteredColors.length} {t('Colors Found', language)}
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                ...
                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-left">{t('Color Detail', language)}</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {loading && colors.length === 0 ? (
                ...
                <tr>
                    <td colSpan="6" className="py-20 text-center text-sm font-bold text-slate-400 uppercase tracking-wider">{t('No colors matching filter', language)}</td>
                </tr>
                  ...
                  <td className="px-6 py-3.5">
                    <div className="flex items-center gap-4 text-left">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] font-black text-slate-400 mt-0.5 tracking-widest uppercase opacity-70">{color.id ? color.id : t(language)}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-3.5">
                    <div className="flex flex-wrap items-center justify-center gap-1.5 max-w-[220px] mx-auto">
                      ...
                      {color.categories?.length > 2 && (
                        <div className="px-2 py-0.5 bg-slate-100 text-slate-400 rounded-lg text-[8px] font-black uppercase tracking-widest whitespace-nowrap">
                          +{color.categories.length - 2} {t('More', language)}
                        </div>
                      )}
                  ...
                  <td className="px-6 py-3.5 text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">
                        {t('at', language)} {new Date(color.updated_at).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </div>
                  </td>
        ...
        <div className="p-4 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-between bg-slate-50/30 gap-4">
           <div className="flex items-center gap-4 order-2 sm:order-1">
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">
                {t('Showing', language)} {paginatedColors.length} {t('of', language)} {filteredColors.length}
              </span>
