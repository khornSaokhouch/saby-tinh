...
export default function AdminProfilePage() {
  ...
  return (
    ...
    >
      ...
      {/* --- 1. EXECUTIVE HEADER --- */}
      <motion.div variants={itemVar} className="relative overflow-hidden rounded-[24px] bg-slate-900 text-white shadow-xl">
        ...
        <div className="absolute bottom-0 left-0 w-40 h-40 bg-blue-500/10 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2 pointer-events-none" />

        <div className="relative p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-5">
          <div className="flex items-center gap-4">
            {/* Avatar */}
            <div className="w-16 h-16 rounded-[20px] bg-slate-800 border-2 border-slate-700 flex items-center justify-center text-2xl font-black text-white shadow-xl overflow-hidden shrink-0">
            ...
            <div className="space-y-1.5">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="px-2.5 py-0.5 rounded-lg bg-indigo-500 text-white text-[9px] font-black uppercase tracking-[0.2em]">
                  {user.role || 'Owner'}
                </span>
              </div>
              <div className="flex flex-wrap items-center gap-3 text-slate-400 text-[10px] font-black uppercase tracking-widest">
                <span className="flex items-center gap-1.5 bg-white/5 px-2.5 py-1 rounded-full border border-white/5"><Mail size={10} className="text-indigo-400" /> {user.email}</span>
                <span className="flex items-center gap-1.5 bg-white/5 px-2.5 py-1 rounded-full border border-white/5"><Building2 size={10} className="text-indigo-400" /> ID: #{user.id}</span>
              </div>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <Link href="/owner/settings" className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-900/50 transition-all flex items-center gap-2 active:scale-95">
              <Settings size={14} strokeWidth={2.5} /> Edit Profile
            </Link>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <motion.div variants={itemVar} className="bg-white border border-slate-100 rounded-[22px] p-6 shadow-sm">
            <div className="flex items-center justify-between mb-5 border-b border-slate-50 pb-4">
              <div className="flex items-center gap-2.5">
                <div className="p-2 bg-indigo-50 text-indigo-600 rounded-xl"><User size={16} strokeWidth={2.5} /></div>
                <h3 className="text-[13px] font-black text-slate-900 tracking-tight">Personal Details</h3>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="md:col-span-2">
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 flex items-center gap-2 ml-1">
                   <Building2 className="w-3 h-3 text-indigo-400" /> Bio
                 </p>
                 <p className="text-[13px] font-medium text-slate-600 leading-relaxed bg-slate-50 p-4 rounded-[18px] border border-slate-100">
                   {user.profile?.bio || "No biography information set."}
                 </p>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVar} className="bg-indigo-600 rounded-[40px] p-10 text-white relative overflow-hidden shadow-2xl shadow-indigo-100 group">
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <button className="px-5 py-2.5 bg-white text-indigo-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-50 transition-all shadow-xl active:scale-95">Manage Financials</button>
                <button className="px-5 py-2.5 bg-transparent border border-white/30 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-colors">View Statements</button>
              </div>
            </div>
            <div className="absolute -left-8 -bottom-8 w-32 h-32 bg-indigo-500/30 rounded-full blur-3xl pointer-events-none" />
          </motion.div>
        ...
        <motion.div variants={itemVar} className="space-y-6">
          ...
          {/* Quick Actions Menu */}
          <div className="bg-white border border-slate-100 rounded-[22px] p-5 shadow-sm h-full flex flex-col">
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 px-1">Quick Actions</h4>
            ...
            <div className="mt-6 pt-5 border-t border-slate-50 space-y-4">
               <div className="p-4 bg-slate-50 rounded-[18px] border border-slate-100">
                  <h5 className="text-[10px] font-black text-slate-800 uppercase tracking-widest mb-1">High Access</h5>
                  <p className="text-[10px] text-slate-400 font-bold leading-relaxed">Any changes you make will be applied system-wide.</p>
               </div>
               <button className="w-full flex items-center justify-center gap-2 py-2.5 text-white bg-rose-500 hover:bg-rose-600 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all active:scale-95">
...
function StatCard({ label, value, icon: Icon, color, trend }) {
  ...
  return (
    <div className="bg-white p-5 rounded-[22px] border border-slate-100 shadow-sm hover:shadow-md transition-all duration-300 group">
      <div className="flex justify-between items-start mb-3">
        <div className={`p-2.5 rounded-xl ${styles[color]}`}>
          <Icon size={17} strokeWidth={2.5} />
        </div>
        <ArrowUpRight className="text-slate-300 group-hover:text-indigo-600 transition-colors" size={13} strokeWidth={3} />
      </div>
      <div>
        <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">{label}</p>
        <h3 className="text-xl font-black text-slate-900 tracking-tighter mt-0.5 capitalize">{value}</h3>
        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">{trend}</p>
      </div>
    </div>
  );
}
...
function DetailItem({ label, value, icon: Icon }) {
  return (
    <div className="group space-y-1.5">
      ...
      <div className="w-full h-11 px-5 bg-slate-50 border border-slate-100 rounded-[16px] flex items-center">
