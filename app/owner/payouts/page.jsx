...
import { 
  Search, RefreshCw, Download, Wallet,
  CalendarDays, Hash, ChevronLeft, ChevronRight, Eye
} from 'lucide-react';
...
export default function OwnerPayouts() {
  ...
  // The backend automatically filters these payouts down to the authenticated user's Store ID
  useEffect(() => {
  ...
  return (
    <div className="space-y-5 pb-8 font-sans max-w-[1400px] mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="text-left">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">{t('Financial Ledger', language)}</span>
          </div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tighter leading-none">
            {t('Payout', language)} <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-400">{t('History', language)}</span>
          </h1>
          <p className="text-slate-500 text-[12px] font-medium mt-1">
            {t('Review your recent store payouts and Bakong settlements.', language)}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard title={t('Total Payouts', language)} value={meta.total || 0} icon={Wallet} color="teal" />
      </div>
      ...
      <div className="bg-white rounded-[20px] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="p-4 border-b border-slate-50 flex flex-col sm:flex-row gap-4 justify-between items-center bg-white">
          <div className="relative w-full sm:w-80 group">
            ...
            <input 
              type="text" 
              placeholder={t('Search by invoice or TRN...', language)} 
          ...
          <table className="w-full text-left whitespace-nowrap">
            <thead>
              <tr className="bg-slate-50/50">
                ...
                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">{t('Invoice / Reference', language)}</th>
                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">{t('Amount Received', language)}</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                ...
                <tr>
                  <td colSpan="5" className="py-20 text-center">
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest leading-none">{t('No payouts found.', language)}</p>
                  </td>
                </tr>
                ...
                <tr key={p.id} className="group hover:bg-slate-50/30 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link href={`/owner/payouts/details?id=${p.id}`}>
        ...
        <div className="p-4 border-t border-slate-50 flex items-center justify-between bg-slate-50/30">
          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
            {t('Page', language)} {meta.current_page || 1} {t('of', language)} {meta.last_page || 1}
          </span>
