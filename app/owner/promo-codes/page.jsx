  ...
  return (
    <div className="space-y-5 pb-8 font-sans max-w-[1400px] mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="text-left">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Merchant Vault</span>
          </div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tighter leading-none">
            Promo <span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-500 to-indigo-500">Codes</span>
          </h1>
          <p className="text-slate-500 text-[12px] font-medium mt-1">Registry status for {today}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <MetricCard label="Total Strings" value={promoCodes.length} icon={Ticket} color="indigo" />
        <MetricCard label="Active Instances" value={promoCodes.filter(p => p.status === 1 || p.status === true).length} icon={CheckCircle2} color="emerald" subText="Live" />
        <MetricCard label="Global Use" value={promoCodes.reduce((acc, p) => acc + (p.usage_count || 0), 0)} icon={RefreshCw} color="rose" />
      </div>
        ...
        <div className="p-4 border-b border-slate-50 bg-slate-50/20">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">
                {filteredPromoCodes.length} Codes verified
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Code Identifier</th>
                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Benefit</th>
                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Activity</th>
                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Status</th>
                <th className="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Activity Log</th>
              </tr>
            </thead>
                ...
                <tr>
                    <td colSpan="5" className="py-20 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Zero strings mapped</p>
                      </div>
                    </td>
                </tr>
                ...
                >
                  <td className="px-6 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col min-w-0">
                        ...
                        <span className="text-[9px] font-black text-slate-300 tracking-widest uppercase mt-0.5 line-clamp-1">{promo.description || 'GENERIC_DESC'}</span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3.5 text-center">
                    <div className="flex flex-col items-center">
                       ...
                       <span className="text-[8px] font-black text-slate-300 uppercase tracking-tighter">Total Hits</span>
                    </div>
                  </td>
                  <td className="px-4 py-3.5 text-center">
                     <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md border text-[8px] font-black uppercase tracking-widest
                        ...
                        {(promo.status === 1 || promo.status === true) ? 'Active' : 'Locked'}
                     </div>
                  </td>
