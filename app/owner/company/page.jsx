...
export default function OwnerCompanyPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-20 font-sans">
      ...
      {/* --- HEADER --- */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-4 backdrop-blur-md z-30 py-4 -my-4 px-1">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight leading-none">Business Profile</h1>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-2">Identity & Logistics Parameters</p>
        </div>
        <button 
          onClick={handleSubmit}
          disabled={isSubmitting}
          className="flex items-center gap-2 px-8 py-4 bg-indigo-600 text-white rounded-[20px] text-[10px] font-black uppercase tracking-[0.2em] shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
        >
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-800">Brand Logo</h3>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-2xl text-white shadow-xl relative overflow-hidden">
             ...
             <div className="relative z-10">
               <div className="flex items-center gap-2 mb-2">
                 ...
                 <span className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em]">Store Profile</span>
               </div>
               <h1 className="text-4xl font-black text-slate-900 tracking-tighter leading-none mb-4">Business Identity</h1>
               <p className="text-xs text-slate-400 leading-relaxed mb-4">
                 Your business details are visible to customers. Ensure location and contact info are accurate.
               </p>
               <button className="text-[10px] font-black text-white bg-white/10 hover:bg-white/20 border border-white/10 px-5 py-3 rounded-xl transition-all flex items-center gap-2 uppercase tracking-widest">
                 View Live Page <ExternalLink size={12} strokeWidth={2.5} />
               </button>
             </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-50">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">Basic information about your business.</p>
              </div>
            </div>
          ...
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-50">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-800">Physical Location</h3>
                <p className="text-xs text-slate-400 font-medium">Address details and map coordinates.</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="h-80 w-full rounded-2xl overflow-hidden border border-slate-200 relative shadow-inner">
                 <div className="absolute top-3 left-3 z-[400] bg-white/90 backdrop-blur px-3 py-2 rounded-lg shadow-sm border border-slate-200 text-[10px] font-bold text-slate-600 flex items-center gap-2">
                   <div className="w-2 h-2 rounded-full bg-rose-500 animate-pulse" />
                   Click map to pin precise location
                 </div>
                 <LocationPicker 
          ...
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-50">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">Connect your social media accounts.</p>
              </div>
            </div>
...
function InputField({ label, name, value, onChange, placeholder, type = "text", icon: Icon, readOnly = false, required = false }) {
  return (
    <div className="relative group">
      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5 ml-1 group-focus-within:text-indigo-600 transition-colors">
      ...
      <div className="relative">
        <input 
          ...
          required={required}
          className={`
            w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 
            focus:outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white 
            transition-all placeholder:text-slate-400 
            ${Icon ? 'pl-10' : ''} 
            ${readOnly ? 'cursor-default bg-slate-100 text-slate-500 focus:border-slate-200 focus:ring-0' : ''}
          `}
        />
        {Icon && (
          <Icon 
            className={`absolute left-3.5 top-1/2 -translate-y-1/2 transition-colors ${readOnly ? 'text-slate-400' : 'text-slate-400 group-focus-within:text-indigo-500'}`} 
            size={16} 
          />
        )}
      </div>
